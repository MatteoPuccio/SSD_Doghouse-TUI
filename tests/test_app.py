...
@mock.patch('requests.post')
def test_app_read_username_receives_valid_username_and_password_call_post_request(mocked_post, valid_username, valid_password):
        app: App = App()
...
@mock.patch('builtins.print')
@mock.patch('requests.post')
def test_app_read_password_receives_invalid_password_fails_show_message_and_do_not_call_post(mocked_post, mocked_print, invalid_password, valid_username):
    app: App = App()
    with patch('builtins.input', side_effect=['1', valid_username,  '0']):
    ...
    with patch('builtins.input', side_effect=['1', valid_username, '0', '0']):
        with patch('getpass.getpass', side_effect=[valid_password]):
            with mock.patch.object(doghousetui.App.App, 'login_request') as mocked_post:
                app: App = App()
                response = Mock(status_code=200)
                response.json.return_value = {"session_token": valid_token, Utils.RESPONSE_ROLE_KEY: Utils.RESPONSE_USER_ROLE_ADMIN_VALUE}
                mocked_post.return_value = response
                app.run()
...
@mock.patch("builtins.print")
def test_app_registration_prints_invalid_password_error_on_invalid_username(mocked_print, invalid_username, valid_username, valid_password):
    with patch('builtins.input', side_effect=['3', invalid_username, valid_username, '0']):
        with patch('getpass.getpass', side_effect=[valid_password, valid_password]):
            with mock.patch.object(doghousetui.App.App, 'make_registration_request') as mocked_post:
                ...
                app: App = App()
                app.run()
                assert mocked_return_args_partial_contains_string_exactly_x_times(mocked_print, Utils.INVALID_USERNAME_ERROR, 1)
...
def test_app_registration_prints_error_on_second_password_different_from_first(mocked_print, valid_username, valid_password):
    with patch('builtins.input', side_effect=['3', valid_username, valid_username, '0']):
        with patch('getpass.getpass', side_effect=[valid_password, valid_password+'a', valid_password, valid_password]):
            with mock.patch.object(doghousetui.App.App, 'make_registration_request') as mocked_post:
                response = Mock(status_code=200)
                mocked_post.return_value = response
                app: App = App()
